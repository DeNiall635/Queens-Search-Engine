...
import (
	...
	searchapi "gitlab2.eeecs.qub.ac.uk/40173800/qse/search/api/search"
...
func main() {
	...
	// Monitoring
	r.Use(chiprometheus.NewMiddleware("search"))
	r.Handle("/metrics", promhttp.Handler())
	// Error handling
	r.NotFound(api.NotFound())
	r.MethodNotAllowed(api.MethodNotAllowed())

	r.Get("/search", searchapi.Search(&searcher))
